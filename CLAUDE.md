...
**A known residual, left in place on purpose and not a licence to add more:** `POST /api/auth/signup` still forwards the provider's `429` (`main.signup`, asserted by `tests/test_signup.py::test_a_provider_rate_limit_is_passed_on_as_one`), which is the same address-keyed signal the reset route collapses — two requests seconds apart distinguish a new address (429, the confirmation mail just went out) from a registered-and-confirmed one (202, obfuscated, no mail and so no cooldown). Reported during step 4 and deferred rather than changed inside a commit scoped to password reset. Fixing it is collapsing that branch into the same 202 and rewriting that assertion.
