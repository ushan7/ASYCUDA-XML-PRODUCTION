...
def _run_provider(role: DeclaredRole, ocr: OcrDocument, fixture: dict | None,
    ...
    if provider == "openai":
        if settings.resolved_openai_key():
            ...
            payload, warnings = OpenAIExtractor().extract(role, ocr, None, deadline=deadline)
            return payload, warnings, "openai"
        ...
        if settings.resolved_openai_key() and langroid_available():
            ...
            return payload, warnings, "langroid"
    ...
    return payload, warnings, "offline"
...
def extract_document(role: DeclaredRole, ocr: OcrDocument, fixture: dict | None = None,
    ...
    try:
        payload, warnings, provider = _run_provider(role, ocr, fixture, deadline)
